...
# Define the ranges for the unidentifiable parameters. The second and third
# elements are zero, which corresponds to assuming conditional independence.
sensitivity_ranges = tibble(
  ranges = list(list(
    lower = c(0.60, 0, 0, 0.15),
    upper = c(0.975, 0, 0, 0.8)
  )),
  range_class = "Main Assumptions",
  cond_ind = TRUE
)

# The best fitting model contains identifiable Frank copulas. We, therefore,
# also assume Frank copulas for the unidentifiable copulas. We also consider two
# information-theoretic definitions of the ICA: the default R_H or the SICC with
# the upper bound transformed to 1.
...
# The ICA_estimator argument is set to NULL for R_H. The default ICA definition
# (i.e., the definition for R_H) will then be used.
